...
/* Select one touch controller configuration. */
// #define TOUCH_FT6X36
// #define TOUCH_FT6X36_SCL 38
// #define TOUCH_FT6X36_SDA 37
// #define TOUCH_FT6X36_INT 4
...
/*
 * GT911 configuration used by this 800x480 panel.
 * The reversed map limits compensate for the panel orientation so LVGL
 * receives coordinates in the same direction as the displayed UI.
 */
 #define TOUCH_GT911
...
/* Most recent mapped point consumed by the LVGL input callback. */
...
#if defined(TOUCH_FT6X36)
/**
 * @brief Convert an FT6X36 touch event into mapped LVGL coordinates.
 *
 * The callback records the latest point and exposes press/release flags for
 * the polling functions used by the LVGL input driver.
 */
...
/**
 * @brief Initialize the selected touch controller and its bus.
 */
void touch_init()
{
#if defined(TOUCH_FT6X36)
...
/**
 * @brief Return whether a touch sample can be read this cycle.
 */
...
/**
 * @brief Read the first touch point and map it to the 800x480 display.
 *
 * @return true when a finger is currently detected; false otherwise.
 */
bool touch_touched()
{
#if defined(TOUCH_FT6X36)
  ...
  {
    /*
     * Map the controller's native coordinate range to the active LVGL display.
     * lcd.width() and lcd.height() keep the mapping tied to the panel geometry.
     */
#if defined(TOUCH_SWAP_XY)
...
/**
 * @brief Report a release event when supported by the selected driver.
 */
bool touch_released()
{
#if defined(TOUCH_FT6X36)
